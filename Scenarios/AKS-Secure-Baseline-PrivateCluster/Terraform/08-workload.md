...
Because the infrastructure has been deployed in a secure manner, the API server to the AKS private cluster is only accessible from within the private network. You will need to perform the majority of the application deployment from the Dev Jumpbox in the Hub VNET, connect via the Bastion Host service. If your computer is connected to the hub network, you may be able to just use that as well.
...
2. Find the Private DNS zone created for azure container registry (Looks like it had old text from 07 MD file and that referes to a KV but in the picture it refers to azureacr). This should be in the landing zone resource group (escs-lz01-rg for example)
...
5. Under **Select principal** click on the **None selected** link and select the user group(s) you created for this to provide you and everyone in the group access to the Key vault (What exact groups are we talking about here ? AKS admin team and AKS app dev team.)
...
```
Why acr name where the kv name should go?
Make sure your user, group, or SP has enough permissions to create the secret
